...
const discordWebhookUrl = process.env.DISCORD_WEBHOOK_URL || "https://discord.com/api/webhooks/<paste-here>"
...
const nucleiWorker = new Worker('nuclei queue', async (job) => {
    ...
    const { stdout, stderr } = await execFile('nuclei', ['-list', `temp/${job.id}`, '-t', `./template/${template.name}.yaml`, '-silent', '-bulk-size', '20', '-c', '15', '-nc', '-o', `${job.id}.json`]);
...
const worker = new Worker('httpx queue', async (job) => {
    ...
    try {
        console.log(`Executing: httpx -l ${filepath} -silent -json -td -title -o ${filepath}.json`);
        await execFile('httpx', ['-l', filepath, '-silent', '-json', '-td', '-title', '-o', filepath + '.json']);
    } catch (error) {
        console.log(error);
    }
